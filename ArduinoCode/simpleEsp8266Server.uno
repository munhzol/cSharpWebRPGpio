...
#ifndef STASSID
#define STASSID "Fios-SP9DH"
#define STAPSK  "jib690wings552tidy"
#endif
...
void setup(void) {
  ...
  server.on("/temp", []() {
    server.send(200, "text/json","{ \"temp\": \""+ String(temprature()) +"\" }" );
  });
...
//    delay(1000);
    ...
    server.send(200, "text/json","{ \"msg\": \"" + server.arg(0) +"\" }" );
  });
...
float temprature(void){

    uint16_t val;
    double dat;
    val=analogRead(A0);//Connect LM35 on Analog 0
    dat = (double) val * (5/10.24) / 10;
